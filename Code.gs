...
function renderPrintableLessonHtml_(lesson, options) {
  ...
  const profile = createPrintLayoutProfile_(lesson, {
  ...
  return [
    ...
    '<div class="print-sheet print-density-' + escapeHtml_(profile.density) + '" data-copy-mode="' + escapeHtml_(copyMode) + '" data-template="' + escapeHtml_(templateType) + '">',

    renderPrintHeaderHtml_(lesson, teacherView, headerInfo, profile),

    renderPrintTemplateBody_(lesson, {
    ...
    teacherView ? renderPrintTeacherNotesSection_(lesson, profile) : '',

    teacherView ? renderPrintAnswerKeySection_(lesson, profile) : '',
...
function createPrintLayoutProfile_(lesson, options) {
  ...
  const densityMap = {
    ...
    GUIDED_PRACTICE: 'standard',

    REVIEW: 'standard',

    QUIZ: 'compact',

    REMEDIATION: 'compact-flow',

    ENRICHMENT: 'compact-flow'

  };

  const answerKeyColumnsMap = {
  ...
  return {
    ...
    density: densityMap[templateType] || 'standard',

    teacherView: !!opts.teacherView,

    copyMode: normalizeEnum_(opts.copyMode, PREVIEW_MODES, 'STUDENT'),

    layoutMode: normalizeEnum_(opts.layoutMode, LAYOUT_MODES, DEFAULT_LAYOUT_MODE),

    answerKeyColumns: answerKeyColumnsMap[templateType] || 2
...
function renderPrintTemplateBody_(lesson, options) {
  ...
  const profile = opts.profile || createPrintLayoutProfile_(lesson, opts);
  ...
  return (renderers[templateType] || renderPrintConceptTemplate_)(lesson, {
...
function renderPrintHeaderHtml_(lesson, teacherView, headerInfo, profile) {
  return `
    <header class="print-header print-keep ${profile && profile.density === 'compact' ? 'print-header-compact' : ''}">
      <div class="print-header-top">
        <div class="print-brand-stack">
          ${headerInfo && headerInfo.schoolName ? `<div class="print-school-name">${escapeHtml_(headerInfo.schoolName)}</div>` : ''}
          ${headerInfo && headerInfo.customHeader ? `<div class="print-custom-header">${escapeHtml_(headerInfo.customHeader)}</div>` : ''}
        </div>
        <div class="print-copy-badge">${teacherView ? 'Teacher Copy' : 'Student Copy'}</div>
      </div>

      <div class="print-title-wrap">
        <div class="print-eyebrow">${escapeHtml_(prettyEnum_(lesson.templateType))}</div>
        <h1 class="print-title">${escapeHtml_(lesson.title)}</h1>
      </div>

      <table
        class="print-meta-table"
        role="presentation"
        style="width:100%; margin:0; border-collapse:separate; border-spacing:8px; table-layout:fixed;"
      >
        <colgroup>
        ...
        <tbody style="display:table-row-group;">
          <tr style="display:table-row;">
            ${renderPrintMetaCell_('Subject', lesson.subject)}
            ${renderPrintMetaCell_('Grade Level', lesson.gradeLevel)}
            ${renderPrintMetaCell_('Topic', lesson.topic)}
          </tr>
          <tr style="display:table-row;">
            ${renderPrintMetaCell_('Template', prettyEnum_(lesson.templateType))}
            ${renderPrintMetaCell_('Difficulty', prettyEnum_(lesson.difficulty))}
            ${renderPrintMetaCell_('Quarter', (headerInfo && headerInfo.quarter) || '')}
          </tr>
          <tr style="display:table-row;">
            ${renderPrintMetaCell_('Teacher Name', (headerInfo && headerInfo.teacherName) || '', 3)}
          </tr>
        </tbody>
      </table>
    </header>
...
function renderPrintMetaCell_(label, value, colspan) {
  const span = colspan && Number(colspan) > 1 ? Number(colspan) : 1;
  const colSpanAttr = span > 1 ? ` colspan="${span}"` : '';
  const widthStyle = span > 1 ? 'width:auto;' : 'width:33.3333%;';

  return `
    <td${colSpanAttr} style="display:table-cell; ${widthStyle} min-width:0; vertical-align:top; padding:10px 12px; border:1px solid #d9e2f0; background:#f7f9fc;">
      <div class="print-meta-label">${escapeHtml_(label)}</div>
      <div class="print-meta-value">${escapeHtml_(value || '')}</div>
    </td>
  `;
}
...
function renderPrintSection_(title, note, bodyHtml, className) {
  ...
  return `
    ...
    <section class="print-section ${className || ''}">
      ...
      <div class="print-section-head print-title-guard">

        <h2 class="print-section-title">${escapeHtml_(title)}</h2>

        ${note ? `<div class="print-section-note">${escapeHtml_(note)}</div>` : ''}
...
function renderPrintObjectiveBlock_(lesson) {

  return `

    <article class="print-panel print-keep">

      <div class="print-panel-label">Objective</div>

      <div class="print-body-text">${escapeHtml_(lesson.objective)}</div>
...
function renderPrintSuccessCriteriaBlock_(lesson) {
  ...
  return `
    ...
    <article class="print-panel print-keep">
      ...
      <div class="print-panel-label">Success Criteria</div>

      <ul class="print-list">

        ${lesson.successCriteria.map(function (item) {
...
function renderPrintVocabularyBlock_(lesson, compact) {
  ...
  const vocabularyHtml = lesson.vocabulary.map(function (item) {

    if (compact) {

      return `

        <li class="print-vocab-row print-keep">

          <span class="print-vocab-term">${escapeHtml_(item.term)}</span>

          <span class="print-vocab-definition">${escapeHtml_(item.definition)}</span>
    ...
    return `
      ...
      <div class="print-vocab-card print-keep">

        <div class="print-vocab-term">${escapeHtml_(item.term)}</div>

        <div class="print-vocab-definition">${escapeHtml_(item.definition)}</div>
  ...
  return compact
    ...
    ? `
      ...
      <article class="print-panel print-keep">
        ...
        <div class="print-panel-label">Vocabulary</div>

        <ul class="print-vocab-list compact">
    ...
    : `
      ...
      <article class="print-panel print-flow">

        <div class="print-panel-label">Vocabulary</div>

        <div class="print-vocab-grid">
...
function renderPrintSummaryBlock_(lesson, label) {
  ...
  return `
    ...
    <article class="print-panel print-flow print-panel-soft">

      <div class="print-panel-label">${escapeHtml_(label || 'Lesson Overview')}</div>

      <div class="print-body-text">${escapeHtml_(lesson.lessonSummary)}</div>
...
function renderPrintConceptCluster_(item, index, config) {

  const cfg = config || {};

  const clusterLabel = cfg.clusterLabel || 'Key Concept';

  const summaryLabel = cfg.summaryLabel || 'Explanation';

  const formulaLabel = cfg.formulaLabel || 'Formula';

  const symbolLabel = cfg.symbolLabel || 'Meaning of Symbols / Important Terms';

  const exampleLabel = cfg.exampleLabel || 'Worked Example';

  const misconceptionLabel = cfg.misconceptionLabel || 'Common Misconception';

  const includeExample = cfg.includeExample !== false;

  const includeMisconception = cfg.includeMisconception !== false && !!item.misconception;
  ...
  return `
    ...
    <div class="print-cluster ${extraClass}" data-concept-index="${index}">

      <div class="print-cluster-head print-title-guard">

        <div class="print-cluster-kicker">${escapeHtml_(clusterLabel)} ${index + 1}</div>

        <h3 class="print-cluster-title">${escapeHtml_(item.heading)}</h3>
      ...
      <article class="print-panel print-flow">
        ...
        <div class="print-panel-label">${escapeHtml_(summaryLabel)}</div>

        <div class="print-body-text">${escapeHtml_(item.summary)}</div>
      ...
      ${item.formula ? `
        ...
        <article class="print-panel print-keep print-panel-accent">

          <div class="print-panel-label">${escapeHtml_(formulaLabel)}</div>

          <div class="print-formula formula-text">${escapeHtml_(item.formula)}</div>
      ...
      ${item.symbolMeaning ? `
        ...
        <article class="print-panel print-keep">
          ...
          <div class="print-panel-label">${escapeHtml_(symbolLabel)}</div>

          <div class="print-body-text">${escapeHtml_(item.symbolMeaning)}</div>
...
${includeExample && item.workedExample ? `

  <article class="print-panel print-flow print-panel-soft print-worked-example">

    <div class="print-panel-label">${escapeHtml_(exampleLabel)}</div>

    <div class="print-body-text">${escapeHtml_(item.workedExample)}</div>
...
` : ''}
      ...
      ${includeMisconception ? `
        ...
        <article class="print-panel print-keep print-panel-warn">

          <div class="print-panel-label">${escapeHtml_(misconceptionLabel)}</div>

          <div class="print-body-text">${escapeHtml_(item.misconception)}</div>
...
function renderPrintCompactReviewCard_(item, index) {

  return `

    <article class="print-panel print-keep print-review-card" data-concept-index="${index}">

      <div class="print-panel-label">Quick Reminder ${index + 1}</div>

      <h3 class="print-card-title">${escapeHtml_(item.heading)}</h3>

      <div class="print-body-text">${escapeHtml_(item.summary)}</div>

      ${item.formula ? `<div class="print-inline-formula formula-text">${escapeHtml_(item.formula)}</div>` : ''}

      ${item.symbolMeaning ? `<div class="print-compact-note"><strong>Remember this:</strong> ${escapeHtml_(item.symbolMeaning)}</div>` : ''}

      ${item.misconception ? `<div class="print-warning-line"><strong>Common error:</strong> ${escapeHtml_(item.misconception)}</div>` : ''}
...
function renderPrintConceptTemplate_(lesson, options) {

  const clustersHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return renderPrintConceptCluster_(item, index, {

      clusterLabel: 'Key Concept',

      summaryLabel: 'Direct Explanation',

      formulaLabel: 'Formula',

      symbolLabel: 'Meaning of Symbols / Important Terms',

      exampleLabel: 'Worked Example',

      misconceptionLabel: 'Common Misconception',

      className: 'print-cluster-concept'
  ...
  const introHtml = [
    ...
    renderPrintObjectiveBlock_(lesson),

    renderPrintSuccessCriteriaBlock_(lesson),

    renderPrintVocabularyBlock_(lesson, false),

    renderPrintSummaryBlock_(lesson, 'Lesson Overview')
  ...
  return [
    ...
    renderPrintSection_('Lesson Frame', 'Core learning targets and language for the lesson.', introHtml, 'print-section-frame'),

    renderPrintSection_('Key Concepts', 'Each concept uses separate print-safe panels so longer explanations can flow without breaking small support boxes.', clustersHtml, 'print-section-concepts'),

    renderPrintPracticeSection_(lesson, {

      teacherView: options && options.teacherView,

      profile: options && options.profile,

      title: 'Individual Practice',

      note: 'Answer all items. Multiple-choice options stay compact, while short answers keep dedicated writing space.',
...
function renderPrintGuidedPracticeTemplate_(lesson, options) {

  const learnHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return renderPrintConceptCluster_(item, index, {

      clusterLabel: 'Learn',

      summaryLabel: 'Key Idea',

      formulaLabel: 'Useful Formula / Rule',

      symbolLabel: 'Key Terms and Symbols',

      includeExample: false,

      misconceptionLabel: 'Watch Out',

      className: 'print-cluster-guided'
  ...
  const modelHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return `

      <div class="print-cluster print-cluster-guided-model" data-guided-model-index="${index}">

        <div class="print-cluster-head print-title-guard">

          <div class="print-cluster-kicker">Practice Model ${index + 1}</div>
...
<article class="print-panel print-flow print-panel-soft print-worked-example">
  ...
  <div class="print-panel-label">Modeled Example</div>
  ...
  const frameHtml = [
    ...
    renderPrintVocabularyBlock_(lesson, true),

    renderPrintSummaryBlock_(lesson, 'Guided Learning Path')
  ...
  return [
    ...
    renderPrintSection_('Guided Lesson Frame', 'The print version keeps the same sequence as the live preview, but with simpler print-safe panels.', frameHtml, 'print-section-frame'),

    renderPrintSection_('Learn', 'Study each idea, then use the matching model before moving to the practice set.', learnHtml, 'print-section-guided-learn'),

    renderPrintSection_('Practice Models', 'Each example is isolated so the example block can move cleanly across pages without forcing the whole concept card to stay together.', modelHtml, 'print-section-guided-models'),
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Complete the tasks independently after studying the models.',
...
function renderPrintReviewTemplate_(lesson, options) {

  const reminderHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return renderPrintCompactReviewCard_(item, index);
  ...
  const frameHtml = [
    ...
    renderPrintSummaryBlock_(lesson, 'Quick Review Focus')
  ...
  return [
    ...
    renderPrintSection_('Review Frame', 'A compact review format for fast scanning before students answer the mixed items.', frameHtml, 'print-section-frame'),

    renderPrintSection_('Quick Reminders', 'Review panels are flattened for print so the sheet stays compact and does not create unstable multi-column gaps.', reminderHtml, 'print-section-review-cards'),
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Use these items to revisit the main ideas with minimal wasted space.',
...
function renderPrintQuizTemplate_(lesson, options) {

  const teacherView = !!(options && options.teacherView);

  const focusHtml = teacherView
  ...
  const infoHtml = `
    ...
    <article class="print-panel print-keep print-quiz-info-panel">

      <div class="print-panel-label">${teacherView ? 'Assessment Record' : 'Student Information'}</div>

      <div class="print-quiz-meta-grid">

        <div class="print-quiz-meta-line"><span>Name</span><span class="print-line-fill"></span></div>

        <div class="print-quiz-meta-line"><span>Date</span><span class="print-line-fill"></span></div>

        <div class="print-quiz-meta-line"><span>Score</span><span class="print-line-fill"></span></div>
  ...
  const directionsHtml = `
    ...
    <article class="print-panel print-keep print-panel-soft">

      <div class="print-panel-label">Directions</div>

      <div class="print-body-text">${teacherView ? 'Use this copy for checking, recording, and quick reference.' : 'Read each item carefully and answer the questions independently.'}</div>

      <ul class="print-list compact">

        <li>Answer every question.</li>

        <li>Review your work before submitting.</li>

        <li>Multiple-choice and true-false items are printed in a compact format to preserve space.</li>
  ...
  return [
    ...
    teacherView ? renderPrintSection_('Assessment Focus', 'Teacher copy includes the checking target without expanding the quiz layout.', focusHtml, 'print-section-quiz-focus') : '',

    renderPrintSection_(teacherView ? 'Assessment Setup' : 'Student Information', '', infoHtml + directionsHtml, 'print-section-quiz-intro'),
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Compact quiz formatting reduces unused vertical space while keeping each item readable.',
...
function renderPrintRemediationTemplate_(lesson, options) {

  const splitIndex = Math.ceil((lesson.practiceItems || []).length / 2);

  const understandHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return renderPrintConceptCluster_(item, index, {

      clusterLabel: 'Understand',

      summaryLabel: 'Understand the Idea',

      formulaLabel: 'Rule / Formula',

      symbolLabel: 'Helpful Terms and Symbols',

      includeExample: false,

      misconceptionLabel: 'Watch Out',

      className: 'print-cluster-remediation'
  ...
  const followHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return `

      <div class="print-cluster print-cluster-remediation-model" data-remediation-model-index="${index}">

        <div class="print-cluster-head print-title-guard">

          <div class="print-cluster-kicker">Follow the Example ${index + 1}</div>
...
<article class="print-panel print-flow print-panel-soft print-worked-example">
  ...
  <div class="print-panel-label">Corrected Reasoning / Model</div>
  ...
  const frameHtml = [
    ...
    renderPrintSummaryBlock_(lesson, 'Support Path')
  ...
  return [
    ...
    renderPrintSection_('Remediation Frame', 'This print path breaks the support sequence into smaller blocks so long explanations can flow without cutting off support cards.', frameHtml, 'print-section-frame'),

    renderPrintSection_('Understand', 'Short support boxes stay together, but the explanation panels are allowed to flow when needed.', understandHtml, 'print-section-remediation-understand'),

    renderPrintSection_('Follow', 'Worked examples are isolated into separate model blocks to avoid one tall monolithic card.', followHtml, 'print-section-remediation-follow'),
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Start with the supported items and use the same steps from the modeled examples.',
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'These items check whether the skill can be used with less support.',

      variant: 'standard',

      startIndex: splitIndex,

      endIndex: (lesson.practiceItems || []).length
...
function renderPrintEnrichmentTemplate_(lesson, options) {

  const splitIndex = Math.ceil((lesson.practiceItems || []).length / 2);

  const deeperHtml = (lesson.keyConcepts || []).map(function (item, index) {

    return renderPrintConceptCluster_(item, index, {

      clusterLabel: 'Think Deeper',

      summaryLabel: 'Big Idea',

      formulaLabel: 'Relationship / Rule',

      symbolLabel: 'Extension Insight',

      exampleLabel: 'Extension Example',

      misconceptionLabel: 'Shortcut to Avoid',

      className: 'print-cluster-enrichment'
  ...
  const frameHtml = [
    ...
    renderPrintSummaryBlock_(lesson, 'Extension Path')
  ...
  return [
    ...
    renderPrintSection_('Enrichment Frame', 'The print layout keeps the premium design language while breaking large enrichment content into smaller print-safe panels.', frameHtml, 'print-section-frame'),

    renderPrintSection_('Think Deeper', 'Big ideas, rules, extension insights, and worked extensions are separated to reduce large blank vertical gaps.', deeperHtml, 'print-section-enrichment-concepts'),
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Use these items to extend the core idea into less familiar situations.',
    ...
    renderPrintPracticeSection_(lesson, {
      ...
      note: 'Use these items to move the idea into a new context or deeper application.',
...
function renderPrintPracticeSection_(lesson, options) {

  const opts = options || {};

  const practiceItems = Array.isArray(lesson.practiceItems) ? lesson.practiceItems : [];

  const startIndex = typeof opts.startIndex === 'number' ? opts.startIndex : 0;

  const endIndex = typeof opts.endIndex === 'number' ? opts.endIndex : practiceItems.length;

  const subset = practiceItems.slice(startIndex, endIndex);
  ...
  const itemsHtml = subset.map(function (item, offset) {

    return renderPrintPracticeItem_(item, startIndex + offset, {
  ...
  return renderPrintSection_(

    opts.title || 'Practice',

    opts.note || '',

    '<div class="print-practice-list print-practice-list-' + escapeHtml_(variant) + '">' + itemsHtml + '</div>',

    'print-section-practice print-section-practice-' + escapeHtml_(variant)
...
function renderPrintPracticeItem_(item, index, options) {

  const opts = options || {};

  const classification = classifyPrintPracticeItem_(item, opts);

  const variantClass = opts.variant === 'quiz' ? ' print-practice-item-quiz' : ' print-practice-item-standard';



  return `

    <article class="print-practice-item ${classification.containerClass}${variantClass}" data-practice-index="${index}">

      <div class="print-practice-head">

        <div class="print-practice-number">Item ${index + 1}</div>

        <div class="print-practice-type">${escapeHtml_(prettyEnum_(item.type))}</div>

      </div>

      <div class="print-practice-question">${escapeHtml_(item.question)}</div>

      ${renderPrintResponseArea_(item, classification, opts.variant || 'standard')}
...
function renderPrintResponseArea_(item, classification, variant) {

  const itemType = normalizeItemType_(item && item.type);



  if (itemType === 'multiple-choice' || itemType === 'true-false') {

    return renderPrintOptionList_(item, variant);

  }



  return renderPrintAnswerSpace_(classification.answerLines, variant === 'quiz');

}



function renderPrintOptionList_(item, variant) {

  const itemType = normalizeItemType_(item && item.type);

  const options = itemType === 'true-false'
  ...
  return `
    ...
    <div class="print-option-list ${variant === 'quiz' ? 'compact' : ''}">

      ${options.map(function (option, optionIndex) {

        const bullet = itemType === 'multiple-choice' ? String.fromCharCode(65 + optionIndex) + '.' : '◯';

        return `

          <div class="print-option-row print-keep">

            <span class="print-option-bullet">${escapeHtml_(bullet)}</span>

            <span class="print-option-text">${escapeHtml_(option)}</span>
...
function renderPrintAnswerSpace_(lineCount, compact) {

  const count = Math.max(1, lineCount || 1);
  ...
  for (let i = 0; i < count; i++) {
    ...
    linesHtml.push('<div class="print-answer-line' + (compact ? ' compact' : '') + '"></div>');

  }



  return '<div class="print-answer-space-block print-keep">' + linesHtml.join('') + '</div>';

}



function classifyPrintPracticeItem_(item, options) {

  const opts = options || {};

  const questionLength = String((item && item.question) || '').length;

  const optionsLength = Array.isArray(item && item.options)

    ? item.options.join(' ').length
  ...
  const isQuiz = opts.variant === 'quiz';

  const keepThreshold = isQuiz ? 260 : 320;

  const answerLines = itemType === 'short-answer'

    ? (totalLength > (isQuiz ? 180 : 230) ? 2 : 1)
  ...
  return {
    ...
    containerClass: totalLength <= keepThreshold ? 'print-keep' : 'print-flow',
...
function renderPrintTeacherNotesSection_(lesson, profile) {
  ...
  return renderPrintSection_(
    ...
    'Reference notes are grouped at the end of the teacher copy so they do not inflate the main print flow.',
    ...
    `
      ...
      <article class="print-panel print-keep">
        ...
        <ul class="print-list">
          ...
          ${lesson.teacherNotes.map(function (note) {

            return `<li>${escapeHtml_(note)}</li>`;
...
function renderPrintAnswerKeySection_(lesson, profile) {

  if (!lesson.practiceItems || !lesson.practiceItems.length) {
  ...
  const answerKeyHtml = lesson.practiceItems.map(function (item, index) {
    ...
    return `
      ...
      <article class="print-answer-key-item print-keep">

        <div class="print-answer-key-number">Item ${index + 1}</div>

        <div class="print-answer-key-question">${escapeHtml_(item.question || '')}</div>

        <div class="print-answer-key-response"><strong>Accepted answer(s):</strong> ${escapeHtml_(answers)}</div>

        ${item.hint ? `<div class="print-answer-key-hint"><strong>Hint:</strong> ${escapeHtml_(item.hint)}</div>` : ''}
  ...
  return renderPrintSection_(
    ...
    'Compact reference blocks replace inline answer boxes in the print items so the assessment pages stay cleaner.',

    '<div class="print-answer-key-grid columns-' + escapeHtml_(String((profile && profile.answerKeyColumns) || 2)) + '">' + answerKeyHtml + '</div>',

    'print-section-answer-key'
...
function buildPrintableDocument_(bodyHtml, options) {
  ...
  return `
    ...
    <html>
      ...
      <head>
        ...
        <style>
          ...
          ${getPrintBaseStyles_(footerText)}
    ...
    <body class="print-mode" data-footer="${escapeHtml_(footerText)}">
...
function getPrintDesignTokensCss_() {
  ...
  return [
    ...
    ':root{--print-text:#172b4d;--print-muted:#5b6b82;--print-line:#dfe3eb;--print-surface:#ffffff;--print-soft:#f8fbff;--print-soft-line:#d9e6ff;--print-warn-bg:#fff6ef;--print-warn-line:#f4c9a6;--print-radius-xl:18px;--print-radius-lg:14px;--print-radius-md:10px;--print-gap-lg:16px;--print-gap-md:12px;--print-gap-sm:8px;--print-block-pad:14px 16px;--print-section-gap:18px;--print-accent:#0052cc;--print-accent-dark:#0f2747;}',

    '.print-sheet[data-template="CONCEPT"]{--print-accent:#0052cc;--print-accent-dark:#0f2747;--print-soft:#f5f9ff;--print-soft-line:#d9e6ff;}',

    '.print-sheet[data-template="GUIDED_PRACTICE"]{--print-accent:#0f766e;--print-accent-dark:#134e4a;--print-soft:#f0fdfa;--print-soft-line:#99f6e4;}',

    '.print-sheet[data-template="REVIEW"]{--print-accent:#7c3aed;--print-accent-dark:#4c1d95;--print-soft:#f5f3ff;--print-soft-line:#ddd6fe;}',

    '.print-sheet[data-template="QUIZ"]{--print-accent:#c2410c;--print-accent-dark:#7c2d12;--print-soft:#fff7ed;--print-soft-line:#fed7aa;}',

    '.print-sheet[data-template="REMEDIATION"]{--print-accent:#be185d;--print-accent-dark:#831843;--print-soft:#fff1f2;--print-soft-line:#fecdd3;}',

    '.print-sheet[data-template="ENRICHMENT"]{--print-accent:#047857;--print-accent-dark:#064e3b;--print-soft:#ecfdf5;--print-soft-line:#a7f3d0;}'
...
function getPrintBaseStyles_(footerText) {

  const safeFooter = escapeCssString_(footerText || '');
  ...
  return [
    ...
    'html,body.print-mode{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;background:#fff !important;color:var(--print-text) !important;font-family:Arial,Helvetica,sans-serif;}',

    '*{box-sizing:border-box;}',

    '.print-doc{width:100%;max-width:none;margin:0 auto;padding:0;}',

    '.print-sheet{width:100%;max-width:none;color:var(--print-text);}',

    '.print-footer{display:none !important;}',

    '.print-header{margin-bottom:20px;padding:18px;border:1px solid var(--print-line);border-top:6px solid var(--print-accent);border-radius:20px;background:linear-gradient(180deg,#ffffff 0%,#fbfcff 100%);break-inside:avoid;page-break-inside:avoid;}',

    '.print-header-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px;}',

    '.print-brand-stack{min-width:0;}',

    '.print-school-name{font-size:13px;font-weight:700;color:var(--print-accent-dark);line-height:1.3;}',

    '.print-custom-header{margin-top:2px;font-size:10px;line-height:1.35;color:var(--print-muted);}',

    '.print-copy-badge{flex:0 0 auto;border:1px solid var(--print-soft-line);background:var(--print-soft);color:var(--print-accent-dark);border-radius:999px;padding:6px 10px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;}',

    '.print-title-wrap{margin-bottom:12px;}',

    '.print-eyebrow{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--print-accent);margin-bottom:5px;}',

    '.print-title{margin:0;font-size:24px;line-height:1.18;color:var(--print-accent-dark);}',

    '.print-meta-table{width:calc(100% + 10px);margin:-5px;border-collapse:separate;border-spacing:10px 10px;table-layout:fixed;}',

    '.print-meta-table td{min-width:0;vertical-align:top;padding:10px 12px;border:1px solid var(--print-soft-line);background:var(--print-soft);border-radius:12px;break-inside:avoid;page-break-inside:avoid;}',

    '.print-meta-label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--print-muted);margin-bottom:4px;}',

    '.print-meta-value{font-size:12px;font-weight:600;line-height:1.35;color:var(--print-text);min-height:1.1em;}',

    '.print-section{margin-bottom:var(--print-section-gap);}',

    '.print-section-head{margin-bottom:10px;break-after:avoid;page-break-after:avoid;}',

    '.print-section-title{margin:0 0 4px;font-size:16px;line-height:1.25;color:var(--print-accent);letter-spacing:.04em;}',

    '.print-section-note{font-size:11px;line-height:1.5;color:var(--print-muted);}',

    '.print-section-body{display:block;}',

    '.print-cluster{margin-bottom:14px;}',

    '.print-cluster-head{margin-bottom:8px;break-after:avoid;page-break-after:avoid;}',

    '.print-cluster-kicker{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--print-accent-dark);margin-bottom:3px;}',

    '.print-cluster-title,.print-card-title{margin:0;font-size:14px;line-height:1.3;color:var(--print-accent-dark);}',

    '.print-panel{margin:0 0 8px;padding:var(--print-block-pad);border:1px solid var(--print-line);border-radius:var(--print-radius-lg);background:var(--print-surface);box-shadow:none;}',

    '.print-panel-accent{border-color:var(--print-soft-line);background:var(--print-soft);}',

    '.print-panel-soft{border-color:var(--print-soft-line);background:color-mix(in srgb,var(--print-soft) 72%, #ffffff 28%);}',

    '.print-panel-warn{border-color:var(--print-warn-line);background:var(--print-warn-bg);}',

    '.print-panel-label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--print-muted);margin-bottom:6px;}',

    '.print-body-text,.print-practice-question,.print-answer-key-question,.print-answer-key-response,.print-answer-key-hint,.print-vocab-definition,.print-warning-line,.print-compact-note{font-size:12px;line-height:1.55;white-space:pre-line;overflow-wrap:anywhere;word-break:break-word;}',

    '.print-formula,.print-inline-formula{font-size:12px;line-height:1.45;white-space:pre-line;overflow-wrap:anywhere;word-break:break-word;}',

    '.print-inline-formula{margin-top:8px;padding:8px 10px;border-radius:10px;background:var(--print-soft);border:1px solid var(--print-soft-line);}',

    '.print-compact-note,.print-warning-line{margin-top:8px;}',

    '.print-list{margin:0;padding-left:18px;font-size:12px;line-height:1.55;}',

    '.print-list.compact li+li{margin-top:4px;}',

    '.print-vocab-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}',

    '.print-vocab-card{padding:10px 12px;border:1px solid var(--print-soft-line);border-radius:12px;background:var(--print-soft);}',

    '.print-vocab-list{margin:0;padding-left:18px;}',

    '.print-vocab-row{font-size:12px;line-height:1.55;}',

    '.print-vocab-row + .print-vocab-row{margin-top:4px;}',

    '.print-vocab-term{font-weight:700;color:var(--print-accent-dark);}',

    '.print-practice-list{display:block;}',

    '.print-practice-item{margin:0 0 10px;padding:12px 14px;border:1px solid var(--print-line);border-radius:14px;background:#fff;}',

    '.print-practice-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px;}',

    '.print-practice-number{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--print-accent-dark);}',

    '.print-practice-type{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--print-muted);text-align:right;}',

    '.print-option-list{display:flex;flex-direction:column;gap:6px;margin-top:8px;}',

    '.print-option-row{display:flex;align-items:flex-start;gap:8px;padding:7px 9px;border:1px solid var(--print-soft-line);border-radius:10px;background:var(--print-soft);}',

    '.print-option-bullet{display:inline-flex;justify-content:center;min-width:20px;font-weight:700;color:var(--print-accent-dark);}',

    '.print-option-text{flex:1;font-size:12px;line-height:1.45;white-space:pre-line;overflow-wrap:anywhere;word-break:break-word;}',

    '.print-answer-space-block{margin-top:10px;}',

    '.print-answer-line{height:18px;border-bottom:1px solid #7a869a;margin-top:9px;}',

    '.print-answer-line:first-child{margin-top:0;}',

    '.print-quiz-meta-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;}',

    '.print-quiz-meta-line{display:flex;align-items:flex-end;gap:8px;min-height:24px;font-size:11px;font-weight:700;color:var(--print-muted);}',

    '.print-line-fill{flex:1;height:16px;border-bottom:1px solid #7a869a;}',

    '.print-answer-key-grid{display:grid;gap:8px;}',

    '.print-answer-key-grid.columns-2{grid-template-columns:repeat(2,minmax(0,1fr));}',

    '.print-answer-key-grid.columns-1{grid-template-columns:1fr;}',

    '.print-answer-key-item{padding:10px 12px;border:1px solid var(--print-soft-line);border-radius:12px;background:var(--print-soft);}',

    '.print-answer-key-number{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--print-accent-dark);margin-bottom:5px;}',

    '.print-answer-key-hint{margin-top:6px;color:var(--print-muted);}',

    '.print-title-guard{break-after:avoid;page-break-after:avoid;}',

'.print-keep,.print-keep *{break-inside:avoid;page-break-inside:avoid;}',
'.print-flow{break-inside:auto;page-break-inside:auto;}',
'.print-flow .print-body-text,.print-flow .print-formula,.print-flow p,.print-flow li{orphans:3;widows:3;}',
'.print-worked-example,.print-worked-example *{break-inside:avoid;page-break-inside:avoid;}',
'.print-worked-example .print-body-text,.print-worked-example .print-formula,.print-worked-example p,.print-worked-example li{orphans:3;widows:3;}',
'.print-cluster.print-worked-example,.print-panel.print-worked-example,.print-panel-accent.print-worked-example,.print-panel-soft.print-worked-example,.print-panel-warn.print-worked-example{break-inside:avoid;page-break-inside:avoid;}',
'.katex-display,.katex,.print-formula,.print-inline-formula{break-inside:avoid;page-break-inside:avoid;}',

    '@media print{.print-section:last-child{margin-bottom:0;}}',

    '@supports not (color: color-mix(in srgb, white 50%, black 50%)){.print-panel-soft{background:var(--print-soft);}}'
...
function getPrintTemplateStyles_() {
  ...
  return [
    ...
    '.print-sheet.print-density-standard{--print-block-pad:14px 16px;--print-section-gap:18px;}',

    '.print-sheet.print-density-compact{--print-block-pad:10px 12px;--print-section-gap:14px;}',

    '.print-sheet.print-density-compact-flow{--print-block-pad:11px 13px;--print-section-gap:15px;}',

    '.print-sheet.print-density-compact .print-header{padding:14px 14px 12px;}',

    '.print-sheet.print-density-compact .print-title{font-size:21px;}',

    '.print-sheet.print-density-compact .print-meta-table{width:calc(100% + 8px);margin:-4px;border-spacing:8px 8px;}',

    '.print-sheet.print-density-compact .print-practice-item{margin-bottom:8px;padding:10px 12px;border-radius:12px;}',

    '.print-sheet.print-density-compact .print-practice-question{font-size:11px;line-height:1.45;}',

    '.print-sheet.print-density-compact .print-option-list{gap:4px;margin-top:6px;}',

    '.print-sheet.print-density-compact .print-option-row{padding:5px 8px;border-radius:9px;}',

    '.print-sheet.print-density-compact .print-option-text{font-size:11px;line-height:1.35;}',

    '.print-sheet.print-density-compact .print-answer-line{height:14px;margin-top:6px;}',

    '.print-sheet.print-density-compact .print-panel-label,.print-sheet.print-density-compact .print-practice-number,.print-sheet.print-density-compact .print-practice-type{font-size:9px;}',

    '.print-sheet.print-density-compact-flow .print-cluster{margin-bottom:12px;}',

    '.print-sheet.print-density-compact-flow .print-panel{margin-bottom:6px;}',

    '.print-sheet[data-template="QUIZ"] .print-practice-item{border-color:#ead9cb;}',

    '.print-sheet[data-template="QUIZ"] .print-option-row{border-color:#edd5c4;background:#fffaf5;}',

    '.print-sheet[data-template="QUIZ"] .print-answer-key-item{background:#fffaf5;border-color:#edd5c4;}',

    '.print-sheet[data-template="REVIEW"] .print-review-card{background:#fff;border-color:var(--print-soft-line);}',

    '.print-sheet[data-template="REVIEW"] .print-section-review-cards .print-section-body{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}',

    '.print-sheet[data-template="GUIDED_PRACTICE"] .print-cluster-guided .print-panel-accent,.print-sheet[data-template="GUIDED_PRACTICE"] .print-cluster-guided-model .print-panel-soft{border-left:4px solid var(--print-accent);}',

    '.print-sheet[data-template="REMEDIATION"] .print-cluster-remediation .print-panel-warn,.print-sheet[data-template="REMEDIATION"] .print-cluster-remediation-model .print-panel-soft{border-left:4px solid var(--print-accent);}',

    '.print-sheet[data-template="ENRICHMENT"] .print-cluster-enrichment .print-panel-accent,.print-sheet[data-template="ENRICHMENT"] .print-cluster-enrichment .print-panel-soft{border-left:4px solid var(--print-accent);}',

    '.print-section-practice-quiz .print-section-note{max-width:40rem;}',

  '@media print and (max-width: 900px){.print-vocab-grid,.print-answer-key-grid.columns-2,.print-sheet[data-template="REVIEW"] .print-section-review-cards .print-section-body,.print-quiz-meta-grid{grid-template-columns:1fr;}}'
